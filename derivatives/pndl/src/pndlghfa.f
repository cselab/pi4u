C  ---------------------------------------------------------------------
      SUBROUTINE PNDLGHFA ( F, X, N, XL, XU, UH, FEPS, IORD, IPRINT,
     &                    GRAD, HES, LD, NOC, IERR )
C  ---------------------------------------------------------------------
C
...
C
C  Output arguments:
C    GRAD       Array containing the resulting Gradient vector.
...
C  ---------------------------------------------------------------------
      ...
      EXTERNAL F
      DIMENSION X(N), XL(N), XU(N), UH(N), GRAD(N), HES(LD,N)
      EXTERNAL PNDL3FF, PNDL3FQ
      include 'torcf.h'
C
C    DIMENSION TMP3(N), TMP4(N), ITMP(N)
C
...
C  desired order of accuracy.
      IF (IORD.EQ.1) THEN
          CALL PNDL3FF(F,X,N,XL,XU,UH,FEPS,IPRINT,GRAD,HES,LD,NOC)
      ELSE IF (IORD.EQ.2) THEN
          CALL PNDL3FQ(F,X,N,XL,XU,UH,FEPS,IPRINT,GRAD,HES,LD,NOC)
      ELSE
          WRITE (*,20) IORD
          IERR = 1
      END IF
