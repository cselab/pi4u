...
void fitfuntask(double in_tparam[], double *out_tparam,	char fitfun_dir[])
{
	...
	if (rf == 0)
	{
		...
		strcpy(args, "");
		sprintf(args, " -stretchingForce=%lf -RBCtotArea=124 -RBCtotVolume=90 -RBCka=4900 -RBCkb=40 -RBCkd=100 -RBCkv=5000 -RBCgammaC=30 -RBCx0=%lf -RBCp=%f -RBCfk=%f", force_F, x0, p_F, fk_F);
		strcat(line, args);
#if DBG
...
double fitfun(double *input, int n, void *output, int *info)
{
	...
	double len		= input[4] * 1e2; // descale
	double fk_F		= input[5];
