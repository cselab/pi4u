...
/*	printf("torc_update_curgen_db_task: %f %f %f %f\n", point[0], point[1], point[2], F); fflush(0); */
	...
	update_curgen_db(point, F, surrogate);
}

void torc_update_curgen_db(double point[], double F, int surrogate)
{
	int me = torc_node_id();
...
void initchaintask(double in_tparam[], int *pdim, double *out_tparam, int winfo[4])
{
	...
	torc_update_curgen_db(point, fpoint, 0);
	torc_update_full_db(point, fpoint, NULL, 0, 0);
...
void chaintask(double in_tparam[], int *pdim, int *pnsteps, double *out_tparam, int *pis_surrogate, int winfo[4])
{
	int i,step;
	int nsteps = *pnsteps;
	int gen_id = winfo[0];
	...
	int leader_is_surrogate, cand_is_surrogate;	/* new, old */
	...
	leader_is_surrogate = *pis_surrogate;
	...
	for (step = 0; step < nsteps; step++) {
		...
		double surr_loglik_candidate = surrogate_estimate(gen_id-1, candidate, data.Nth);

		int is_good = surrogate_is_good_estimate(surr_loglik_candidate);
		if (is_good) {
			cand_is_surrogate = 1;	// yes, it is
			loglik_candidate = surr_loglik_candidate;
#if 1	// This is for testing/debugging
			{
			double real_loglik_candidate;
			evaluate_F(candidate, &real_loglik_candidate, me, gen_id, chain_id, step, 1);
			dec_nfc();

			double minv, maxv, abs_er, rel_er;
			abs_er = fabs(real_loglik_candidate - surr_loglik_candidate);
			minv = MIN(fabs(real_loglik_candidate),fabs(surr_loglik_candidate));
			maxv = MIN(fabs(real_loglik_candidate),fabs(surr_loglik_candidate));
			if (minv == 0.0 && maxv == 0)	rel_er = 0;
			else if (minv == 0.0)		rel_er = 100.0*abs_er/maxv;
			else				rel_er = 100.0*abs_er/minv;

			printf("A gen %d chain %d step %d: loglik_candidate vs surr_loglik_candidate : %f vs %f (%.2f%%)\n",
				gen_id, chain_id, step, loglik_candidate, surr_loglik_candidate, rel_er);
			}
#endif
		}
		else	{
			cand_is_surrogate = 0;	// no, it is not (not needed)
			evaluate_F(candidate, &loglik_candidate, me, gen_id, chain_id, step, 1);

			double minv, maxv, abs_er, rel_er;
			abs_er = fabs(loglik_candidate - surr_loglik_candidate);
			minv = MIN(fabs(loglik_candidate),fabs(surr_loglik_candidate));
			maxv = MIN(fabs(loglik_candidate),fabs(surr_loglik_candidate));
			if (minv == 0.0 && maxv == 0)	rel_er = 0;
			else if (minv == 0.0)		rel_er = 100.0*abs_er/maxv;
			else				rel_er = 100.0*abs_er/minv;

			printf("B gen %d chain %d step %d: loglik_candidate vs surr_loglik_candidate : %f vs %f (%.2f%%)\n",
				gen_id, chain_id, step, loglik_candidate, surr_loglik_candidate, rel_er);

		}

		torc_update_full_db(candidate, loglik_candidate, NULL, 0, cand_is_surrogate);	// 
		...
		if (P < L) {
			...
			leader_is_surrogate = cand_is_surrogate;	/* whatever the new point is  */
			torc_update_curgen_db(leader, loglik_leader, leader_is_surrogate);

		}
		else {
			/*increase counter or add the leader again in curgen_db*/
			torc_update_curgen_db(leader, loglik_leader, leader_is_surrogate);	// was it a surrogate?
		}
	}
...
#if 1 // NN
			leaders[ldi].surrogate = curgen_db.entry[idx].surrogate;
			//leaders[ldi].error = surrogate_error_estimate(runinfo.Gen-1, leaders[ldi].point, data.Nth, leaders[ldi].F);	// I don't think this is useful
#endif
			ldi++;
		}
	}
...
//		printf("%d %d %f %d\n", i, leaders[i].nsel, leaders[i].F, leaders[i].queue);
#if 1 // NN
//		printf("%d %d %f %d (%d %f)\n", i, leaders[i].nsel, leaders[i].F, leaders[i].queue, leaders[i].surrogate, leaders[i].error);
		printf("%d %d %f %d (%d)\n", i, leaders[i].nsel, leaders[i].F, leaders[i].queue, leaders[i].surrogate);
#endif
	}
#endif
...
int main(int argc, char *argv[])
{
	...
	dump_full_db(runinfo.Gen);
#if 1	// NN
	surrogate_build(runinfo.Gen);
#endif
...
/*	spmd_print_matrix_2d();*/
	...
	for (; runinfo.Gen < data.MaxStages; runinfo.Gen++){
		...
		for (i = 0; i < nchains; i++) {
			...
			is_surrogate = leaders[i].surrogate;
			//printf("xx: %d spawning chain for leader %d: %d %f\n", runinfo.Gen, i, leaders[i].surrogate, leaders[i].error);
			torc_create(leaders[i].queue, chaintask, 6,
				...
				1, MPI_INT, CALL_BY_COP,
				4, MPI_INT, CALL_BY_COP,
				in_tparam, &data.Nth, &nsteps, &out_tparam[i], &is_surrogate, winfo);
