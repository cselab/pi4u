...
void update_curgen_db(double point[], double F, int surrogate)
{
	int PROBDIM = data.Nth;
	int i, pos;
	...
	curgen_db.entry[pos].surrogate = surrogate;
}
...
void dump_full_db(int Gen)
{
	...
	for (pos = 0; pos < full_db.entries; pos++) {
		...
		fprintf(fp, "%20.16lf ", full_db.entry[pos].F);
		fprintf(fp, "%d\n", full_db.entry[pos].surrogate);
	}
	fclose(fp);
}
...
int load_full_db(int Gen)
{
	...
	for (pos = 0; pos < full_db.entries; pos++) {
		int i;
		for (i = 0; i < PROBDIM; i++) {
			if (full_db.entry[pos].point == NULL) full_db.entry[pos].point = malloc(PROBDIM*sizeof(double));
			fscanf(fp, "%lf", &full_db.entry[pos].point[i]);
		}
		fscanf(fp, "%lf", &full_db.entry[pos].F);
		fscanf(fp, "%d", &full_db.entry[pos].surrogate);
	}
	fclose(fp);
